...
function startPlayer2() {
    get(1, movePlayer2);
// Listen for bullet changes for both Player 2 and Player 1
listenForBulletChanges(1, (bullet) => {
    // Player 2's bullet
    players[1].bullets.push(bullet);
});
listenForBulletChanges(0, (bullet) => {
    // Player 1's bullet is visible to Player 2
    players[1].bullets.push(bullet);
});
    ...
    document.addEventListener("keydown", function (event) {
        ...
        if (players[1].id === 1) {
            ...
            if (event.key === "w" && players[1].y > 0) {
                ...
                const bullet = { x: players[1].x + players[1].width, y: players[1].y + players[1].height / 2, direction: 1 };
                players[1].bullets.push(bullet);
                saveBulletToFirebase(1, bullet); // Save and synchronize bullet movement
         }  // Save the updated bullets to Firebase
